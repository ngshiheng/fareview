...
ENVIRONMENT := $(shell echo $${ENVIRONMENT-development})
PG_HOST := $(shell echo $${PG_HOST-$$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.Gateway}}{{end}}' dpostgres)})
...
.PHONY: build
build:	## build docker image for fareview
	@$(DOCKER) build -t $(NAME) . --build-arg ENVIRONMENT=$(ENVIRONMENT)

.PHONY: run
run:	## run fareview in docker
